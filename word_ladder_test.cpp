/*
  ...
  Explain and justify how you approached testing, the degree
   ...
   Cover the basic scenarios, smaller ladders and ensure the basic functionality
   of the interface provided works as intended. This makes test scenarios simple
   which makes checking implementation correctness simpler. Also, if the smaller
   cases work as intended, then it can be assumed the bigger cases also will and
   only time and space complexity will be the issue.
...
SCENARIO("creating the graph") {
  ...
  std::unordered_set<std::string> lexicon {"word", "rope", "cat", "failure",
                                           "hope", "role"};
  std::string src = "rope";
...
SCENARIO("base cases for bfs") {
  ...
  GIVEN("a source word and a graph made from a lexicon") {
    std::unordered_set<std::string> lexicon {"word", "rope"};
    std::string src = "rope";
    auto graph = CreateGraph(lexicon, src);
    ...
    GIVEN("an unreachable destination from the source word") {
      ...
      WHEN("finding the shortest word ladders using bfs") {
        ...
        THEN("no word ladder is returned") {
          REQUIRE(word_ladders.size() == 0);
        }
      }
    }
  }
  ...
  GIVEN("two words and a graph made from a lexicon") {
    ...
    WHEN("finding the shortest word ladders using bfs") {
      ...
      THEN("the difference between adjacent words is one letter") {
        REQUIRE(word_ladders == std::vector<std::vector<std::string>> {
          {"word", "wore", "ware", "rare"}
        });
      }
    }
